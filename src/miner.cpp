...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("UrkoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UrkoinMiner : ProcessBlock, block not accepted");
    }
...
{
    LogPrintf("UrkoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("urkoin-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running UrkoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("UrkoinMiner terminated\n");
        throw;
    }
}
